...
export function createMcpServer(kernel, browserPool, { paperKernel, paperContentKernel, paperCacheStore, paperCacheCleanup } = {}) {
  ...
  server.registerTool('get_weather', {
    title: 'Get Weather',
    description: 'Get current conditions and 7-day forecast for a location (Open-Meteo, free, no key). Input: pass the full place name directly as one string, e.g. Chinese "北京", "上海三林", "成都武侯区", "三林镇"; English "Beijing", "London, UK"; or coordinates "51.51, -0.13". IMPORTANT for Chinese: a "city+sub-area" like "上海三林" is passed as one string — the tool auto-resolves to the sub-area (三林镇); do NOT split it into province/city/district hierarchy. Chinese names are romanized/OSM-resolved internally. When a name matches multiple same-named places, the tool returns a numbered list with coordinates — then you MUST re-call get_weather with the chosen entry\'s coordinates (e.g. "31.23, 121.47") or a more specific name; do not stop at the list. Examples: "北京" → direct; "上海三林" → 三林镇; "51.51, -0.13" → coordinates.',
    inputSchema: {
      location: z.string().min(1).describe('Location: full place name or coordinates ("lat, lon"). Chinese "city+sub-area" like "上海三林" is fine as one string (auto-resolved). If a previous call returned a numbered location list, re-call with the chosen coordinates or a more specific name.')
    }
  }, wrapHandler(async (args) => {
